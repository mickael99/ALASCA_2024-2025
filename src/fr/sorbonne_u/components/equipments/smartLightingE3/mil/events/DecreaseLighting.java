...
public class DecreaseLighting extends ES_Event implements SmartLightingEventI {
    // ------------------------------------------------------------------------
    // Constants
    // ------------------------------------------------------------------------
    ...
    public void executeOn(AtomicModelI model) {
        ...
        new NeoSim4JavaException(
                "DecreaseLighting: Precondition violation: model instanceof "
        ...
        if (model instanceof SmartLightingElectricityModel) {
            ...
            assert m.getState() == SmartLightingElectricityModel.State.ON ||
                   m.getState() == SmartLightingElectricityModel.State.INCREASE;
            new NeoSim4JavaException(
                    "DecreaseLighting: Precondition violation: m.getState() == "
                    + "HeaterIlluminanceModel.State.ON or DECREASE but is "
                    + m.getState()
            );

            m.setState(SmartLightingElectricityModel.State.DECREASE, this.getTimeOfOccurrence());
        } else {
            ...
            assert m.getState() == SmartLightingIlluminanceModel.State.ON ||
                   m.getState() == SmartLightingIlluminanceModel.State.INCREASE;
            new NeoSim4JavaException(
                    "DecreaseLighting: Precondition violation: m.getState() == "
                    + "HeaterIlluminanceModel.State.ON or DECREASE but is "
                    + m.getState()
            ...
            m.setState(SmartLightingIlluminanceModel.State.DECREASE);
        }
    }
}

...
import fr.sorbonne_u.components.equipments.smartLighting.connections.SmartLightingActuatorInboundPort;
import fr.sorbonne_u.components.equipments.smartLighting.connections.SmartLightingSensorDataInboundPort;
...
public class SmartLighting extends AbstractCyPhyComponent
    implements SmartLightingUserI, SmartLightingInternalControlI {
  ...
  public static final String INTERNAL_CONTROL_INBOUND_PORT_URI =
      "SMART-LIGHTING-INTERNAL-CONTROL-INBOUND-PORT-URI";
  public static final String EXTERNAL_CONTROL_INBOUND_PORT_URI =
      "SMART-LIGHTING-EXTERNAL-CONTROL-INBOUND-PORT-URI";
  ...
  protected static boolean glassBoxInvariants(SmartLighting sm) {
    ...
    ret &=
        InvariantChecking.checkGlassBoxInvariant(
            MAX_POWER_LEVEL >= 0, SmartLighting.class, sm, "MAX_POWER_LEVEL >= 0");
    ret &=
        InvariantChecking.checkGlassBoxInvariant(
            sm.currentState != null, SmartLighting.class, sm, "sm.currentState != null");
    ret &=
        InvariantChecking.checkGlassBoxInvariant(
            sm.targetIllumination >= 0, SmartLighting.class, sm, "sm.targetIllumination >= 0");
    ret &=
        InvariantChecking.checkGlassBoxInvariant(
            sm.currentPowerLevel >= 0, SmartLighting.class, sm, "sm.currentPowerLevel >= 0");
    ret &=
        InvariantChecking.checkGlassBoxInvariant(
    ...
    ret &=
        InvariantChecking.checkGlassBoxInvariant(
            sm.currentSimulationType.isNoSimulation()
                || (sm.globalArchitectureURI != null && sm.globalArchitectureURI.isEmpty()),
            SmartLighting.class,
            sm,
  ...
  protected static boolean blackBoxInvariants(SmartLighting sm) {
    ...
    ret &=
        InvariantChecking.checkBlackBoxInvariant(
            ILLUMINATION_UNIT != null, SmartLighting.class, sm, "ILLUMINATION_UNIT != null");
    ret &=
        InvariantChecking.checkBlackBoxInvariant(
    ...
    ret &=
        InvariantChecking.checkBlackBoxInvariant(
            X_RELATIVE_POSITION >= 0, SmartLighting.class, sm, "X_RELATIVE_POSITION >= 0");
    ret &=
        InvariantChecking.checkBlackBoxInvariant(
            Y_RELATIVE_POSITION >= 0, SmartLighting.class, sm, "Y_RELATIVE_POSITION >= 0");
  ...
  protected SmartLighting(
    ...
    assert !executionType.isStandard() || clockURI != null || !clockURI.isEmpty()
        : new PreconditionException(
            "!executionType.isStandard() || clockURI != null || !clockURI.isEmpty()");
    assert !executionType.isStandard() || simulationType.isNoSimulation()
        : new PreconditionException(
            "!executionType.isStandard() || simulationType.isNoSimulation()");
    assert simulationType.isNoSimulation()
            || (globalArchitectureURI != null && !globalArchitectureURI.isEmpty())
        : new PreconditionException(
            "simulationType.isNoSimulation() || (globalArchitectureURI != null && !globalArchitectureURI.isEmpty())");
    assert simulationType.isNoSimulation()
            || (localArchitectureURI != null && !localArchitectureURI.isEmpty())
        : new PreconditionException(
            "simulationType.isNoSimulation() || (localArchitectureURI != null && !localArchitectureURI.isEmpty())");
    ...
    assert SmartLighting.glassBoxInvariants(this)
        : new InvariantException("glassBoxInvariants(this)");
    assert SmartLighting.blackBoxInvariants(this)
  ...
  protected void initialise(
    ...
    assert smartLightingUserInboundPortURI != null && !smartLightingUserInboundPortURI.isEmpty()
        : new PreconditionException(
            "smartLightingUserInboundPortURI != null && !smartLightingUserInboundPortURI.isEmpty()");
    assert smartLightingInternalControlInboundPortURI != null
            && !smartLightingInternalControlInboundPortURI.isEmpty()
        : new PreconditionException(
            "smartLightingInternalControlInboundPortURI != null && !smartLightingInternalControlInboundPortURI.isEmpty()");
    assert smartLightingExternalControlInboundPortURI != null
            && !smartLightingExternalControlInboundPortURI.isEmpty()
        : new PreconditionException(
            "smartLightingExternalControlInboundPortURI != null && !smartLightingExternalControlInboundPortURI.isEmpty()");
    assert smartLightingSensorInboundPortURI != null && !smartLightingSensorInboundPortURI.isEmpty()
        : new PreconditionException(
            "smartLightingSensorInboundPortURI != null && !smartLightingSensorInboundPortURI.isEmpty()");
    assert smartLightingActuatorInboundPortURI != null
            && !smartLightingActuatorInboundPortURI.isEmpty()
        : new PreconditionException(
            "smartLightingActuatorInboundPortURI != null && !smartLightingActuatorInboundPortURI.isEmpty()");
    ...
    this.slcip =
        new SmartLightingInternalControlInboundPort(
            smartLightingInternalControlInboundPortURI, this);
    this.slcip.publishPort();
    this.sleip =
        new SmartLightingExternalControlInboundPort(
            smartLightingExternalControlInboundPortURI, this);
    this.sleip.publishPort();

    this.sensorDataInboundPort =
        new SmartLightingSensorDataInboundPort(smartLightingSensorInboundPortURI, this);
    this.sensorDataInboundPort.publishPort();
    this.actuatorInboundPort =
        new SmartLightingActuatorInboundPort(smartLightingActuatorInboundPortURI, this);
  ...
  public synchronized void start() throws ComponentStartException {
    ...
    try {
      ...
      switch (this.currentSimulationType) {
        ...
        case SIL_SIMULATION:
          this.asp =
              new RTAtomicSimulatorPlugin() {
                private static final long serialVersionUID = 1L;

                /**
                 * @see
                 *     fr.sorbonne_u.components.cyphy.plugins.devs.AtomicSimulatorPlugin#getModelStateValue(java.lang.String,
                 *     java.lang.String)
                 */
                @Override
                public Object getModelStateValue(String modelURI, String name) throws Exception {
                  ...
                  return ((SmartLightingIlluminanceModel)
                          this.atomicSimulators.get(modelURI).getSimulatedModel())
  ...
  public synchronized void execute() throws Exception {
    if (this.isHEMConnectionRequired && TEST_REGISTRATION) this.runAllRegistrationTest();
    if (this.isHEMConnectionRequired) {
      if (!this.currentExecutionType.isStandard()) {
        ...
        AcceleratedAndSimulationClock clock = clockServerOBP.getClockWithSimulation(this.clockURI);
        ...
        if (this.currentExecutionType.isUnitTest()
          ...
          this.asp.initialiseSimulation(
              new Time(clock.getSimulatedStartTime(), clock.getSimulatedTimeUnit()),
              new Duration(clock.getSimulatedDuration(), clock.getSimulatedTimeUnit()));
        }

        long simulationStartTimeInMillis =
            TimeUnit.NANOSECONDS.toMillis(clock.getSimulationStartEpochNanos());
        assert simulationStartTimeInMillis > System.currentTimeMillis()
            : new BCMException("simulationStartTimeInMillis > " + "System.currentTimeMillis()");
  ...
  public synchronized void shutdown() throws ComponentShutdownException {
    try {
      ...
      // Registration
      if (this.isHEMConnectionRequired) this.registrationPort.unpublishPort();
  ...
  public synchronized void finalise() throws Exception {
    // Registration
    if (this.isHEMConnectionRequired) this.doPortDisconnection(this.registrationPort.getPortURI());
  ...
  public boolean isOn() throws Exception {
    ...
    return this.currentState == SmartLightingState.ON
  ...
  public void setTargetIllumination(double targetIllumination) throws Exception {
    if (SmartLighting.VERBOSE) {
      this.traceMessage(
          "SmartLighting sets its target illumination to " + targetIllumination + ".\n");
    }

    assert this.isOn() : new PreconditionException("SmartLighting is off.");

    assert targetIllumination >= 0.0
        : new PreconditionException("Target illumination must be positive.");

    this.targetIllumination = targetIllumination;

    assert this.getTargetIllumination() == targetIllumination
        : new PostconditionException("Target illumination has not been set correctly.");
  }
  ...
  public double getMaxPowerLevel() throws Exception {
    if (SmartLighting.VERBOSE) {
      this.traceMessage(
          "SmartLighting returns its maximum power level: " + MAX_POWER_LEVEL + ".\n");
    }
  ...
  public double getCurrentPowerLevel() throws Exception {
    if (SmartLighting.VERBOSE) {
      this.traceMessage(
          "SmartLighting returns its current power level: " + this.currentPowerLevel + ".\n");
    }
    ...
    assert powerLevel >= 0.0 && powerLevel <= MAX_POWER_LEVEL
        : new PostconditionException("Current Power level is not in the correct range.");
  ...
  public void setCurrentPowerLevel(double powerLevel) throws Exception {
    ...
    assert this.isOn() : new PreconditionException("SmartLighting is off.");
    assert powerLevel >= 0.0 && powerLevel <= MAX_POWER_LEVEL
        : new PreconditionException("Power level must be in the correct range.");

    this.currentPowerLevel = powerLevel;

    assert this.getCurrentPowerLevel() == powerLevel
        : new PostconditionException("Current power level has not been set correctly.");
  }
  ...
  public void IncreaseLightIntensity() throws Exception {
    ...
    assert this.isOn() : new PreconditionException("SmartLighting is off.");
    assert !this.isSwitchingAutomatically()
        : new PreconditionException("SmartLighting is already in mode automatically.");

    this.currentState = SmartLightingState.INCREASE;

    assert this.isSwitchingAutomatically()
        : new PostconditionException("SmartLighting is not in mode automatically.");
  }
  ...
  public boolean isSwitchingAutomatically() throws Exception {
    ...
    return this.currentState == SmartLightingState.INCREASE
        || this.currentState == SmartLightingState.DECREASE;
  }
  ...
  public double getTargetIllumination() throws Exception {
    ...
    if (SmartLighting.VERBOSE) {
      this.traceMessage(
          "SmartLighting returns its target illumination: " + this.targetIllumination + ".\n");
    }

    double targetIllumination = this.targetIllumination;

    assert targetIllumination >= 0.0
        : new PostconditionException("Target illumination is not in the correct range.");
  ...
  public double getCurrentIllumination() throws Exception {
    ...
    if (SmartLighting.VERBOSE) {
      this.traceMessage(
          "SmartLighting returns its current illumination: " + this.targetIllumination + ".\n");
    }
  ...
  public void startIlluminationPushSensor(long controlPeriod, TimeUnit tu) throws Exception {
    ...
    if (this.currentExecutionType.isStandard()) {
      ...
      actualControlPeriod = (long) ((controlPeriod * tu.toNanos(1)) / ac.getAccelerationFactor());
  ...
  protected void IlluminationPushSensor() throws Exception {
    this.sensorDataInboundPort.send(
        new SmartLightingSensorData<SmartLightingCompoundMeasure>(
  ...
  protected void testRegistered() {
    if (VERBOSE) this.traceMessage("Test registered\n");
    try {
    ...
    if (VERBOSE) this.traceMessage("Done...\n");
  }

  protected void testRegister() {
    if (VERBOSE) this.traceMessage("Test register\n");
    try {
  ...
  protected void testUnregister() {
    if (VERBOSE) this.traceMessage("Test unregister\n");
    try {
